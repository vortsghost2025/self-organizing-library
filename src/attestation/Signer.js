 ...
 * inbox messages, and continuity records using RSA private keys.
 *
...
class Signer {
  ...
  /**
   * Sign an inbox message per v1.1 lane-relay protocol.
   * The signable payload includes critical identity fields:
   * task_id, from, to, timestamp, priority, type, task_kind, lane.
   * The JWS is verified on the receiving end by IdentityEnforcer.
   *
   * @param {object} msg - Inbox message object (v1.1 schema)
   * @param {string|Buffer} privateKey - RSA private key (PEM or KeyObject)
   * @param {string} keyId - Key identifier for trust store lookup
   * @returns {object} Message with signature, signature_alg, key_id fields
   */
  signInboxMessage(msg, privateKey, keyId) {
    const lane = msg.from || msg.from_lane; // Canonical field
    ...
    const signablePayload = {
      ...
      lane: lane // Ensures consistency with Verifier's A=B=C lane check
    };
