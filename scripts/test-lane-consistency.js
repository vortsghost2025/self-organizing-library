...
try {
  ...
  const trustPath = (constants.ARCHIVIST_TRUST_STORE_PATH || constants.TRUST_STORE_PATH || '').replace(/\\/g, '/');
  const hasLocalTrustStore = trustPath && (trustPath.includes('library/lanes/broadcast/trust-store.json') || trustPath.includes('Archivist-Agent/lanes/broadcast/trust-store.json') || trustPath.includes('self-organizing-library/lanes/broadcast/trust-store.json'));
